...
class CNCControlApp:
    ...
    def crear_interfaz(self):
        ...
        # Nuevos botones de Control de Motor (Spindle)
        tk.Button(self.frame_manual, text="Encender Motor", bg="#ccffcc", command=lambda: self.enviar_comando_grbl("M3 S1000", False)).grid(row=3, column=0, pady=5)
        tk.Button(self.frame_manual, text="Apagar Motor", bg="#ffcccc", command=lambda: self.enviar_comando_grbl("M5", False)).grid(row=3, column=1, pady=5)
        # 3. Archivos y Ruteo
            ...
            if coords:
                ...
                if todos_x and todos_y:
                    ...
                    for trazo in self.coords_crudas:
                        ...
                        if es_borde_placa:
                            self.coords_borde_placa.append(trazo)
                        else:
                ...
                for trazo in self.coords_borde_placa:
                    for i in range(len(trazo)-1):
                        x1 = (trazo[i][0] * self.escala_ref) + self.offset_x_ref
                        y1 = self.offset_y_ref - (trazo[i][1] * self.escala_ref)
                        x2 = (trazo[i+1][0] * self.escala_ref) + self.offset_x_ref
    ...
    def generar_gcode_aislamiento(self, tipo_broca, diametro, angulo, z_corte, z_seguro, feedrate, num_pasadas=1):
        ...
        if hasattr(self, 'coords_borde_placa'):
            for trazo in self.coords_borde_placa:
   ...
   # =========================================================================
    ...
    def iniciar_ruteo(self):
        self.btn_iniciar.config(state=tk.DISABLED)
        self.btn_limpiar.config(state=tk.DISABLED) 
        self.tiempo_inicio_ruteo = time.time()
    ...
    def hilo_enviar_gcode(self):
       ...
       # --- 2. MENSAJE AL FINALIZAR CON ÉXITO ---
        if completado:
            ...
            # --- NUEVO: CÁLCULO DE TIEMPO Y LOG PARA LA TESIS ---
            tiempo_total = time.time() - self.tiempo_inicio_ruteo
            minutos = int(tiempo_total // 60)
            segundos = int(tiempo_total % 60)

            import os
            ruta_carpeta_actual = os.path.dirname(os.path.abspath(__file__))
            ruta_archivo_log = os.path.join(ruta_carpeta_actual, "Resultados_Tesis_Log.txt")

            with open(ruta_archivo_log, "a", encoding="utf-8") as log:
                log.write(f"Archivo: {self.ruta_archivo_actual} | Tiempo: {minutos}m {segundos}s\n")

            # Actualizamos la ventana de éxito para que muestre el tiempo y la ruta
            mensaje_final = f"El proceso de la placa ha finalizado con éxito.\n\nTiempo total: {minutos} minutos y {segundos} segundos.\n\nRegistro guardado en:\n{ruta_archivo_log}"
